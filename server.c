...
const char *get_file_extension(const char *file_name) {
    ...
    return dot + 1; //extension without '.'
}

// text/html - type/subtype
const char *get_mime_type(const char *file_ext) {
    // checking and mapping extension
...
void build_http_response(const char *file_name, const char *file_ext,
    ...
    // format and store values into a header,
...
//    off_t file_size = file_stat.st_size; // file size
    ...
    *response_len += strlen(header);
    ...
    ssize_t bytes_read;
    //The file is read in chunks and appended to the response buffer until the entire file is read
    while ((bytes_read = read(file_fd,
                              response + *response_len,
                              BUFFER_SIZE - *response_len)) > 0) {
        *response_len += bytes_read;
    }
    // closing the file descriptor and freeing the allocated memory for the header.
    free(header);
    close(file_fd);
}
...
int main() {
    ...
    server_addr.sin_addr.s_addr = INADDR_ANY; // 0.0.0.0 - any address
